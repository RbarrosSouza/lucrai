  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center justify-between gap-3 flex-wrap">
        <div>
          <p className="text-xs font-semibold text-gray-900">Planejamento</p>
        ...
        <div className="flex items-center gap-2 flex-wrap justify-end">
          ...
          {!isEditing ? (
            <button
              onClick={onStartEdit}
              className="flex items-center gap-2 bg-lucrai-500 hover:bg-lucrai-600 text-white px-3 py-1.5 rounded-lg text-xs font-bold transition-colors"
            ...
            <div className="flex gap-2">
              {periodMode === 'MONTH' ? (
                <button
                  ...
                  className="flex items-center gap-2 px-3 py-1.5 rounded-lg bg-white border border-gray-200 text-gray-700 font-bold text-xs hover:bg-gray-50 disabled:opacity-60"
                  disabled={saving}
              ...
              <button
                ...
                className="px-3 py-1.5 bg-white border border-gray-200 text-gray-700 rounded-lg hover:bg-gray-50 text-xs font-bold"
                disabled={saving}
              >
              ...
              <button
                onClick={onSave}
                className="px-3 py-1.5 bg-lucrai-500 hover:bg-lucrai-600 text-white rounded-lg text-xs font-bold disabled:opacity-60"
                disabled={saving}
              >
      ...
      <div className="bg-gray-50 border border-gray-200 rounded-xl overflow-hidden">
        <div className="grid grid-cols-12 gap-0 px-3 py-2 text-[11px] font-bold text-gray-500 uppercase tracking-wide border-b border-gray-200">
        ...
        <div className="divide-y divide-gray-200 bg-white">
          {tree.map((dre) => {
            ...
            return (
              <div key={dre.id}>
                <div className="grid grid-cols-12 gap-0 px-3 py-2 items-center bg-gray-50/70">
                  <div className="col-span-6 min-w-0">
                    <button type="button" onClick={() => toggleExpand(`dre:${dre.id}`)} className="flex items-center gap-2 min-w-0 w-full text-left">
                      ...
                      <div className="min-w-0">
                        <div className="text-xs font-bold text-gray-900 truncate">{dre.label}</div>
                        <div className="text-[11px] text-gray-500">DRE</div>
                      </div>
                    </button>
                  </div>
                  <div className="col-span-3 text-right text-xs font-semibold text-gray-900">{formatMoney(dre.realized)}</div>
                  <div className="col-span-3 text-right">
                    <div className="text-xs font-bold text-gray-900">{formatMoney(dre.budget)}</div>
                ...
                {dreOpen
                  ? dre.children.map((cat) => {
                      ...
                      return (
                        <div key={cat.id}>
                          <div className="grid grid-cols-12 gap-0 px-3 py-2 items-center bg-white">
                            <div className="col-span-6 min-w-0">
                              <button
                                type="button"
                              ...
                              >
                                ...
                                <div className="min-w-0">
                                  <div className="text-xs font-semibold text-gray-900 truncate">{cat.label}</div>
                                  <div className="text-[11px] text-gray-500">Categoria</div>
                                </div>
                              </button>
                            </div>
                            <div className="col-span-3 text-right text-xs font-medium text-gray-900">{formatMoney(cat.realized)}</div>
                            <div className="col-span-3 text-right">
                              <div className="text-xs font-semibold text-gray-900">{formatMoney(cat.budget)}</div>
                          ...
                          {catOpen
                            ? cat.children.map((cc) => {
                                ...
                                return (
                                  <div key={cc.id} className="grid grid-cols-12 gap-0 px-3 py-2 items-center hover:bg-gray-50">
                                    <div className="col-span-6 min-w-0" style={{ paddingLeft: 44 }}>
                                      <div className="flex items-center justify-between gap-2">
                                        <div className="min-w-0">
                                          <div className="text-xs font-semibold text-gray-900 truncate">{cc.label}</div>
                                    ...
                                    <div className="col-span-3 text-right text-xs font-medium text-gray-900">{formatMoney(cc.realized)}</div>
                                    <div className="col-span-3 text-right">
                                      {isEditing ? (
                                        <input
                                          type="number"
                                          ...
                                          className="w-28 text-right px-2 py-1.5 rounded-lg border border-gray-200 focus:ring-2 focus:ring-lucrai-200 focus:border-lucrai-500 outline-none text-xs font-semibold text-gray-900"
                                        />
                                      ) : (
                                        <div className="text-xs font-semibold text-gray-900">{formatMoney(cc.budget)}</div>
                                      )}
        ...
        <div className="fixed inset-0 z-[80] flex items-end sm:items-center justify-center p-0 sm:p-4">
          ...
          <div className="relative w-full sm:max-w-2xl bg-white border border-gray-200 shadow-2xl rounded-t-3xl sm:rounded-3xl overflow-hidden">
            <div className="p-4 border-b border-gray-100 flex items-center justify-between">
              <div>
                <div className="text-xs font-bold text-gray-900">Distribuição mensal</div>
              ...
              {distMonths.map((m) => (
                <div key={m} className="bg-gray-50 border border-gray-200 rounded-xl p-3">
                  ...
                  <input
                    ...
                    className="mt-2 w-full px-3 py-2 rounded-lg bg-white border border-gray-200 text-xs font-semibold text-gray-900 focus:outline-none focus:ring-2 focus:ring-lucrai-200"
                  />
                </div>
              ))}
            </div>
            ...
            <div className="p-4 border-t border-gray-100 bg-gray-50/50 flex justify-end gap-2">
              <button type="button" onClick={closeDist} className="px-3 py-1.5 rounded-xl bg-white border border-gray-200 text-gray-700 font-bold">
                Fechar
              </button>
              <button type="button" onClick={closeDist} className="px-3 py-1.5 rounded-xl bg-lucrai-500 hover:bg-lucrai-600 text-white font-bold">
                Ok
              </button>
            </div>
          </div>
